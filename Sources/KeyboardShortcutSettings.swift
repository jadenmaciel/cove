...
enum KeyboardShortcutSettings {
    static let didChangeNotification = Notification.Name("cove.keyboardShortcutSettingsDidChange")
    static let actionUserInfoKey = "action"
    static let settingsFileDisplayPath = "~/.config/cove/cove.json"
    ...
    enum Action: String, CaseIterable, Identifiable {
        ...
        case switchRightSidebarToInspector
        case triggerFlash
        ...
        var label: String {
            ...
            case .quit: return String(localized: "menu.quitCove", defaultValue: "Quit cove")
            ...
            case .switchRightSidebarToInspector: return String(localized: "shortcut.switchRightSidebarToInspector.label", defaultValue: "Show Sidebar Inspector")
            ...
            case .nextWorkspace: return String(localized: "shortcut.nextWorkspaceCycle.label", defaultValue: "Next Workspace (Cycle)")
            case .prevWorkspace: return String(localized: "shortcut.previousWorkspaceCycle.label", defaultValue: "Previous Workspace (Cycle)")
        ...
        func normalizedSettingsFileShortcut(_ shortcut: StoredShortcut) -> StoredShortcut? {
            // cove.json can load while the global settings store is still initializing.
...
final class SystemWideHotkeyController {
...
#if DEBUG
        if status != noErr {
            coveDebugLog("globalHotkey.handlerInstall failed status=\(status)")
        }
#endif
    }
...
#if DEBUG
        coveDebugLog("globalHotkey.fire shortcut=\(shortcut.displayString) active=\(NSApp.isActive ? 1 : 0)")
#endif
    ...
    private func toggleApplicationVisibility() {
        // Only treat the hotkey as a "hide" toggle when cove itself is the
        // frontmost app and has at least one visible window. If the user
        // pressed the hotkey from another app, cove is not frontmost (even if
        // some of its windows are still on screen) and the expected behavior
        // is to bring cove forward, not hide it.
...
enum KeyboardShortcutRecorderActivity {
    static let didChangeNotification = Notification.Name("cove.keyboardShortcutRecorderActivityDidChange")
    static let stopAllNotification = Notification.Name("cove.keyboardShortcutRecorderActivityStopAll")
