...
fileprivate func coveVsyncIOSurfaceTimelineCallback(
...
class TabManager: ObservableObject {
    ...
    /// Global monotonically increasing counter for COVE_PORT ordinal assignment.
    ...
    private let initialWorkspaceGitProbeQueue = DispatchQueue(
        label: "com.cove.initial-workspace-git-probe",
        qos: .utility
    )
    ...
    func makeWorkspaceForCreation(
        ...
        portOrdinal: Int,
        configTemplate: CoveSurfaceConfigTemplate?,
    ...
    func maybeMutateSelectionDuringWorkspaceCreationForDev(
        ...
        let isEnabled: Bool = {
            if let raw = env["COVE_DEV_MUTATE_WORKSPACE_SELECTION_DURING_CREATION"] {
                return raw == "1" || raw.caseInsensitiveCompare("true") == .orderedSame
            }
            return UserDefaults.standard.bool(forKey: "coveDevMutateWorkspaceSelectionDuringCreation")
        }()
        guard isEnabled,
    ...
    func addWorkspace(
        ...
        return withExtendedLifetime((capturedTabs, sourceWorkspace)) {
            ...
            // Register permanent workspace and capture initial layout snapshot
            registerPermanentWorkspace(newWorkspace)

            return newWorkspace
        }
    }

    // MARK: - Permanent Workspace Registration

    /// Registers a newly created workspace in the WorkspaceRegistry and saves its initial layout snapshot.
    @MainActor
    private func registerPermanentWorkspace(_ workspace: Workspace) {
        guard let registryURL = WorkspaceRegistryStore.defaultRegistryFileURL() else { return }
        ...
        let record = PermanentWorkspaceRecord(
            id: workspace.id,
            kind: .project,
            name: Self.liveProjectWorkspaceName(for: workspace),
            rootPath: workspace.currentDirectory,
            pinned: false,
            sortIndex: registry.workspaces.count,
        ...
        _ = WorkspaceRegistryStore.save(registry, fileURL: registryURL)

        // Capture and save initial layout snapshot
        let snapshot = workspace.sessionSnapshot(includeScrollback: false)
        if let layoutURL = layoutFileURL(for: workspace.id, registryURL: registryURL) {
            _ = SessionPersistenceStore.saveWorkspaceLayout(snapshot, fileURL: layoutURL)
        }
    }

    /// Returns the per-workspace layout file URL within the Application Support directory.
    private func layoutFileURL(for workspaceId: UUID, registryURL: URL) -> URL? {
        let baseDirectory = registryURL.deletingLastPathComponent()
        return baseDirectory
            .appendingPathComponent("sessions/workspaces/\(workspaceId.uuidString)/layout.json", isDirectory: false)
    }

    /// Restores permanent workspaces from the WorkspaceRegistry.
    /// Returns the restored workspace IDs for the caller to use for selection.
    @MainActor
    func restorePermanentWorkspaces() -> [UUID] {
        guard let registryURL = WorkspaceRegistryStore.defaultRegistryFileURL() else { return [] }
        guard let registry = WorkspaceRegistryStore.load(fileURL: registryURL) else { return [] }

        let records = registry.defaultVisibleWorkspaces
        ...
        for record in records {
            ...
            // Load and restore layout snapshot
            if let layoutURL = layoutFileURL(for: record.id, registryURL: registryURL),
               let snapshot = loadWorkspaceLayout(from: layoutURL) {
                workspace.restoreSessionSnapshot(snapshot)
            }

            wireClosedBrowserTracking(for: workspace)
            tabs.append(workspace)
            restoredIds.append(workspace.id)
        }

        return restoredIds
    }

    /// Loads a workspace layout snapshot from the given URL.
    private func loadWorkspaceLayout(from url: URL) -> SessionWorkspaceSnapshot? {
        return SessionPersistenceStore.loadWorkspaceLayout(from: url)
    }
        ...
        if let terminalPanel = workspace.focusedTerminalPanel,
           terminalPanel.surface.surface != nil {
            DispatchQueue.main.asyncAfter(deadline: .now() + 0.5) {
                ...
                terminalPanel.sendText("cove welcome\n")
            }
            return
        }
...
#if DEBUG
                coveDebugLog("workspace.prRefresh.repo.fail repo=\(repoSlug) page=\(page) status=nil")
#endif
                return .transientFailure
            }
...
#if DEBUG
                coveDebugLog("workspace.prRefresh.repo.fail repo=\(repoSlug) page=\(page) status=\(response.statusCode)")
#endif
                return .transientFailure
            }
...
#if DEBUG
            coveDebugLog("workspace.prRefresh.branch.fail repo=\(repoSlug) branch=\(branch) status=nil")
#endif
            return .transientFailure
        }
    ...
    private nonisolated static func performWorkspacePullRequestRequest(
        ...
        request.setValue("cove-workspace-pr-poller", forHTTPHeaderField: "User-Agent")
    ...
    private func inheritedTerminalConfigForNewWorkspace() -> CoveSurfaceConfigTemplate? {
    ...
    /// Deletes a permanent workspace: kills all terminals, archives the record, and closes the tab.
    @MainActor
    func deleteWorkspace(_ workspace: Workspace) {
        let title = String(localized: "dialog.deleteWorkspace.title", defaultValue: "Delete Workspace?")
        let message = String(
            localized: "dialog.deleteWorkspace.message",
            defaultValue: "All terminals in this workspace will be closed."
        )
        guard confirmClose(title: title, message: message, acceptCmdD: false) else { return }

        // D-11: kill all terminals
        workspace.teardownAllPanels()

        // Archive the permanent workspace record
        if let registryURL = WorkspaceRegistryStore.defaultRegistryFileURL(),
           var registry = WorkspaceRegistryStore.load(fileURL: registryURL) {
            if let index = registry.workspaces.firstIndex(where: { $0.id == workspace.id }) {
                registry.workspaces[index].archive()
                _ = WorkspaceRegistryStore.save(registry, fileURL: registryURL)
            }
        }

        // Remove from sidebar selection
        sidebarSelectedWorkspaceIds.remove(workspace.id)
    ...
    /// Renames a permanent workspace in the registry.
    @MainActor
    func renamePermanentWorkspace(tabId: UUID, name: String) {
        guard let registryURL = WorkspaceRegistryStore.defaultRegistryFileURL(),
              var registry = WorkspaceRegistryStore.load(fileURL: registryURL) else { return }
        if let index = registry.workspaces.firstIndex(where: { $0.id == tabId }) {
            registry.workspaces[index].name = name
...
#if DEBUG
        if ProcessInfo.processInfo.environment["COVE_UI_TEST_FORCE_CONFIRM_CLOSE_WORKSPACE"] == "1" {
            return true
        }
#endif
    ...
    private func windowTitle(for tab: Workspace?) -> String {
        guard let tab else { return "cove" }
        ...
        return trimmedDirectory.isEmpty ? "cove" : trimmedDirectory
    }
...
#if DEBUG
            coveDebugLog("notification.focus.fail tab=\(tabId.uuidString.prefix(5)) reason=missingTab")
#endif
            return false
        }
        if let surfaceId, tab.panels[surfaceId] == nil {
    ...
    func applySurfaceTabBarButtons(
        _ buttons: [CoveSurfaceTabBarButton],
        sourcePath: String?,
        globalConfigPath: String,
        terminalCommandSourcePaths: [String: String],
        workspaceCommands: [String: CoveResolvedCommand]
    ) {
        for workspace in tabs {
    ...
    private func setupUITestFocusShortcutsIfNeeded() {
        ...
        let env = ProcessInfo.processInfo.environment
        guard env["COVE_UI_TEST_FOCUS_SHORTCUTS"] == "1" else { return }
    ...
    private func setupSplitCloseRightUITestIfNeeded() {
        ...
        let env = ProcessInfo.processInfo.environment
        guard env["COVE_UI_TEST_SPLIT_CLOSE_RIGHT_SETUP"] == "1" else { return }
        guard let path = env["COVE_UI_TEST_SPLIT_CLOSE_RIGHT_PATH"], !path.isEmpty else { return }
        let visualMode = env["COVE_UI_TEST_SPLIT_CLOSE_RIGHT_VISUAL"] == "1"
        let shotsDir = (env["COVE_UI_TEST_SPLIT_CLOSE_RIGHT_SHOTS_DIR"] ?? "").trimmingCharacters(in: .whitespacesAndNewlines)
        let visualIterations = Int((env["COVE_UI_TEST_SPLIT_CLOSE_RIGHT_ITERATIONS"] ?? "20").trimmingCharacters(in: .whitespacesAndNewlines)) ?? 20
        let burstFrames = Int((env["COVE_UI_TEST_SPLIT_CLOSE_RIGHT_BURST_FRAMES"] ?? "6").trimmingCharacters(in: .whitespacesAndNewlines)) ?? 6
        let closeDelayMs = Int((env["COVE_UI_TEST_SPLIT_CLOSE_RIGHT_CLOSE_DELAY_MS"] ?? "70").trimmingCharacters(in: .whitespacesAndNewlines)) ?? 70
        let pattern = (env["COVE_UI_TEST_SPLIT_CLOSE_RIGHT_PATTERN"] ?? "close_right")
            .trimmingCharacters(in: .whitespacesAndNewlines)
        ...
        for i in 1...iterations {
            ...
            sendText(topLeftId, "printf '\\033[2J\\033[H'; for i in {1..200}; do echo COVE_SPLIT_TOPLEFT_\(i); done; printf '\\033[HCOVE_MARKER_TOPLEFT\\n'\r")
            sendText(topRight.id, "printf '\\033[2J\\033[H'; for i in {1..200}; do echo COVE_SPLIT_TOPRIGHT_\(i); done; printf '\\033[HCOVE_MARKER_TOPRIGHT\\n'\r")
            if let bottomLeft {
                sendText(bottomLeft.id, "printf '\\033[2J\\033[H'; for i in {1..200}; do echo COVE_SPLIT_BOTTOMLEFT_\(i); done; printf '\\033[HCOVE_MARKER_BOTTOMLEFT\\n'\r")
            }
            if let bottomRight {
                sendText(bottomRight.id, "printf '\\033[2J\\033[H'; for i in {1..200}; do echo COVE_SPLIT_BOTTOMRIGHT_\(i); done; printf '\\033[HCOVE_MARKER_BOTTOMRIGHT\\n'\r")
            }
	    ...
	    private func captureVsyncIOSurfaceTimeline(
	        ...
	        await withCheckedContinuation { (cont: CheckedContinuation<Void, Never>) in
	            ...
	            CVDisplayLinkSetOutputCallback(link, coveVsyncIOSurfaceTimelineCallback, ctx)
    ...
    private func setupChildExitSplitUITestIfNeeded() {
        ...
        let env = ProcessInfo.processInfo.environment
        guard env["COVE_UI_TEST_CHILD_EXIT_SPLIT_SETUP"] == "1" else { return }
        guard let path = env["COVE_UI_TEST_CHILD_EXIT_SPLIT_PATH"], !path.isEmpty else { return }
        let requestedIterations = Int(env["COVE_UI_TEST_CHILD_EXIT_SPLIT_ITERATIONS"] ?? "1") ?? 1
    ...
    private func setupChildExitKeyboardUITestIfNeeded() {
        ...
        let env = ProcessInfo.processInfo.environment
        guard env["COVE_UI_TEST_CHILD_EXIT_KEYBOARD_SETUP"] == "1" else { return }
        guard let path = env["COVE_UI_TEST_CHILD_EXIT_KEYBOARD_PATH"], !path.isEmpty else { return }
        let autoTrigger = env["COVE_UI_TEST_CHILD_EXIT_KEYBOARD_AUTO_TRIGGER"] == "1"
        let strictKeyOnly = env["COVE_UI_TEST_CHILD_EXIT_KEYBOARD_STRICT"] == "1"
        let triggerMode = (env["COVE_UI_TEST_CHILD_EXIT_KEYBOARD_TRIGGER_MODE"] ?? "shell_input")
            .trimmingCharacters(in: .whitespacesAndNewlines)
        ...
        let layout = (env["COVE_UI_TEST_CHILD_EXIT_KEYBOARD_LAYOUT"] ?? "lr")
            .trimmingCharacters(in: .whitespacesAndNewlines)
        let expectedPanelsAfter = max(
            1,
            Int((env["COVE_UI_TEST_CHILD_EXIT_KEYBOARD_EXPECTED_PANELS_AFTER"] ?? "1")
                .trimmingCharacters(in: .whitespacesAndNewlines)
            ) ?? 1
        )
...
extension Notification.Name {
    static let commandPaletteToggleRequested = Notification.Name("cove.commandPaletteToggleRequested")
    static let commandPaletteRequested = Notification.Name("cove.commandPaletteRequested")
    static let commandPaletteSwitcherRequested = Notification.Name("cove.commandPaletteSwitcherRequested")
    static let commandPaletteSubmitRequested = Notification.Name("cove.commandPaletteSubmitRequested")
    static let commandPaletteDismissRequested = Notification.Name("cove.commandPaletteDismissRequested")
    static let commandPaletteRenameTabRequested = Notification.Name("cove.commandPaletteRenameTabRequested")
    static let commandPaletteRenameWorkspaceRequested = Notification.Name("cove.commandPaletteRenameWorkspaceRequested")
    static let commandPaletteEditWorkspaceDescriptionRequested = Notification.Name("cove.commandPaletteEditWorkspaceDescriptionRequested")
    static let commandPaletteMoveSelection = Notification.Name("cove.commandPaletteMoveSelection")
    static let commandPaletteRenameInputInteractionRequested = Notification.Name("cove.commandPaletteRenameInputInteractionRequested")
    static let commandPaletteRenameInputDeleteBackwardRequested = Notification.Name("cove.commandPaletteRenameInputDeleteBackwardRequested")
    static let feedbackComposerRequested = Notification.Name("cove.feedbackComposerRequested")
    ...
    static let terminalPortalVisibilityDidChange = Notification.Name("cove.terminalPortalVisibilityDidChange")
    static let browserPortalRegistryDidChange = Notification.Name("cove.browserPortalRegistryDidChange")
}
