...
enum DragOverlayRoutingPolicy {
    ...
    static let filePreviewTransferType = NSPasteboard.PasteboardType("com.cove.filepreview.transfer")
    ...
    static func hasInternalCoveDrag(_ pasteboardTypes: [NSPasteboard.PasteboardType]?) -> Bool {
    ...
    static func shouldCaptureFileDropDestination(
        ...
        guard !hasInternalCoveDrag(pasteboardTypes) else { return false }
        return true
    }
...
let commandPaletteOverlayContainerIdentifier = NSUserInterfaceItemIdentifier("cove.commandPalette.overlay.container")
let tmuxWorkspacePaneOverlayContainerIdentifier = NSUserInterfaceItemIdentifier("cove.tmuxWorkspacePane.overlay.container")
...
private final class WindowCommandPaletteOverlayController: NSObject {
    ...
    private func scheduleFocusIntoPalette(retries: Int = 4) {
        ...
        if let window {
            ...
            coveDebugLog("palette.focus.schedule retries=\(retries) window=nil")
        }
#endif
    ...
    func update(
        ...
        if let window {
            ...
            coveDebugLog("palette.overlay.update visible=\(isVisible ? 1 : 0) promote=\(shouldPromote ? 1 : 0) window=nil")
        }
#endif
...
struct ContentView: View {
    ...
    @EnvironmentObject var coveConfigStore: CoveConfigStore
    ...
    /// Native titlebar inset reported by AppKit. Standard mode follows cove's visual chrome;
    ...
    private struct CovePanelGlassOverlay: View {
        var body: some View {
    ...
    private var rightSidebarPanel: some View {
        return RightSidebarPanelView(
            ...
            workspaceRecord: selectedWorkspaceRecordForInspector,
    ...
    @MainActor
    private var selectedWorkspaceRecordForInspector: PermanentWorkspaceRecord? {
        let registry = WorkspaceRegistry.liveProjectWorkspaceProjection(
            from: tabManager.tabs,
            selectedWorkspaceID: tabManager.selectedTabId
        )
        guard let selectedId = tabManager.selectedTabId else { return nil }
        return registry.workspaces.first { $0.id == selectedId }
    }
    ...
    private var windowIdentifier: String { "cove.main.\(windowId.uuidString)" }
    ...
    private func refreshWindowChromeMetrics(for window: NSWindow) {
        ...
        // Standard mode uses cove's visual chrome height for layout.
    ...
    private func syncFileExplorerDirectory() {
        ...
        if tab.isRemoteWorkspace {
            ...
            #if DEBUG
            coveDebugLog("fileExplorer.sync remote dir=\(dir) ready=\(isReady) dest=\(config?.destination ?? "nil")")
            #endif
    ...
    var body: some View {
        ...
        view = AnyView(view.onAppear {
            ...
            DispatchQueue.main.asyncAfter(deadline: .now() + 0.5) { [weak tabManager] in
                ...
                if didRecover {
#if DEBUG
                    coveDebugLog("startup.recovery tabCount=\(tabManager.tabs.count) selected=\(tabManager.selectedTabId?.uuidString.prefix(8) ?? "nil") mounted=\(mountedWorkspaceIds.count)")
#endif
        ...
        view = AnyView(view.onChange(of: tabManager.selectedTabId) { newValue in
            ...
            if let snapshot = tabManager.debugCurrentWorkspaceSwitchSnapshot() {
                ...
                coveDebugLog("ws.view.selectedChange id=none selected=\(debugShortWorkspaceId(newValue))")
            }
#endif
            tabManager.applyWindowBackgroundForSelectedTab()
        ...
        view = AnyView(view.onChange(of: tabManager.isWorkspaceCycleHot) { _ in
            ...
            if let snapshot = tabManager.debugCurrentWorkspaceSwitchSnapshot() {
                ...
                coveDebugLog("ws.view.hotChange id=none hot=\(tabManager.isWorkspaceCycleHot ? 1 : 0)")
            }
#endif
            reconcileMountedWorkspaceIds()
        })
        ...
        view = AnyView(view.background(WindowAccessor(refreshID: appearance.appKitWindowMutationID) { [appearance] window in
...
#if DEBUG
            if ProcessInfo.processInfo.environment["COVE_UI_TEST_MODE"] == "1" {
            ...
            // Let cove supply the translucent titlebar fills. AppKit's native
            ...
            AppDelegate.shared?.registerMainWindow(
                ...
                fileExplorerState: fileExplorerState,
                coveConfigStore: coveConfigStore
            )
            installFileDropOverlayWhenReady(on: window, tabManager: tabManager)
        }))

        return AnyView(view.coveAppearanceColorScheme(appearanceMode))
    }
    ...
    private func primeBackgroundWorkspaceIfNeeded(workspaceId: UUID) async {
        ...
        coveDebugLog("workspace.backgroundPrime.start workspace=\(workspaceId.uuidString.prefix(5))")
#endif
    ...
    private func removeNativeTitlebarBackdrop(in window: NSWindow) {
        ...
        let identifier = NSUserInterfaceItemIdentifier("cove.nativeTitlebarBackdrop")
    ...
    private func setTitlebarControlsHidden(_ hidden: Bool, in window: NSWindow) {
        let controlsId = NSUserInterfaceItemIdentifier("cove.titlebarControls")
    ...
    private func startWorkspaceHandoffIfNeeded(newSelectedId: UUID?) {
        ...
        if canCompleteWorkspaceHandoffImmediately(for: newSelectedId) {
            ...
            if let snapshot = tabManager.debugCurrentWorkspaceSwitchSnapshot() {
                ...
                coveDebugLog("ws.handoff.fastReady id=none selected=\(debugShortWorkspaceId(newSelectedId))")
            }
#endif
    ...
    private func completeWorkspaceHandoff(reason: String) {
        ...
        if let snapshot = tabManager.debugCurrentWorkspaceSwitchSnapshot() {
            ...
            coveDebugLog("ws.handoff.complete id=none reason=\(reason) retiring=\(debugShortWorkspaceId(retiring))")
        }
#endif
    }
    ...
    private final class CommandPaletteMultilineTextEditorView: NSView {
        ...
        func focusIfNeeded() {
            guard let window else {
#if DEBUG
                coveDebugLog("palette.wsDescription.editor.focusIfNeeded window=nil")
#endif
                return
            }
...
#if DEBUG
                        coveDebugLog("palette.wsDescription.editor.handleKeyEvent action=submit")
                        coveDebugLog(
...
#if DEBUG
                        coveDebugLog("palette.wsDescription.editor.handleKeyEvent action=allowShiftReturn")
#endif
                        return false
                    }
                }
...
#if DEBUG
                    coveDebugLog("palette.wsDescription.editor.handleKeyEvent action=escape")
#endif
                    parent.onEscape()
                    return true
                }
...
#if DEBUG
                coveDebugLog("palette.wsDescription.editor.handleKeyEvent action=passThrough")
#endif
                return false
            }
        }
    ...
    private func commandPaletteCommandsFingerprint(commandsContext: CommandPaletteCommandsContext) -> Int {
        ...
        hasher.combine(coveConfigStore.configRevision)
    ...
    private func commandPaletteCommandsContext(
        ...
        let cliInstalledInPATH = AppDelegate.shared?.isCoveCLIInstalledInPATH() ?? false
    ...
    private func commandPaletteCommands(
        ...
        for contribution in contributions {
            let configuredPaletteAction = commandPaletteConfigActionID(for: contribution.commandId)
                .flatMap { coveConfigStore.resolvedAction(id: $0) }
    ...
    private func commandPaletteConfigActionID(for commandId: String) -> String? {
        ...
        case "palette.newTerminalTab":
            return CoveSurfaceTabBarBuiltInAction.newTerminal.configID
        case "palette.newBrowserTab":
            return CoveSurfaceTabBarBuiltInAction.newBrowser.configID
        case "palette.terminalSplitRight":
            return CoveSurfaceTabBarBuiltInAction.splitRight.configID
        case "palette.terminalSplitDown":
            return CoveSurfaceTabBarBuiltInAction.splitDown.configID
        default:
            return nil
        }
    }
    ...
    private func commandPaletteShortcutHint(
        ...
        if let configuredShortcut = coveConfigStore.resolvedAction(id: contribution.commandId)?.shortcut {
        ...
        if let configuredPaletteAction = commandPaletteConfigActionID(for: contribution.commandId),
           let configuredShortcut = coveConfigStore.resolvedAction(id: configuredPaletteAction)?.shortcut {
            return configuredShortcut.displayString
        }
    ...
    private func commandPaletteCommandContributions() -> [CommandPaletteCommandContribution] {
        ...
        contributions.append(
            CommandPaletteCommandContribution(
                ...
                title: constant(String(localized: "command.installCLI.title", defaultValue: "Shell Command: Install 'cove' in PATH")),
        ...
        contributions.append(
            CommandPaletteCommandContribution(
                ...
                title: constant(String(localized: "command.uninstallCLI.title", defaultValue: "Shell Command: Uninstall 'cove' from PATH")),
        ...
        contributions.append(
            CommandPaletteCommandContribution(
                commandId: "palette.openCoveSettingsFile",
                title: constant(String(localized: "settings.settingsJSON.openFile", defaultValue: "Open cove.json")),
                subtitle: constant(String(localized: "command.coveConfig.subtitle", defaultValue: "cove.json")),
                keywords: ["open", "cove", "json", "config", "configuration", "settings", "file", "editor", "dotfile"]
            )
        )
        contributions.append(
            CommandPaletteCommandContribution(
        ...
        contributions.append(
            CommandPaletteCommandContribution(
                ...
                keywords: ["restart", "socket", "listener", "cli", "cove", "control"]
            )
        )
        contributions.append(
            CommandPaletteCommandContribution(
                commandId: "palette.disableBrowser",
                title: constant(String(localized: "command.disableBrowser.title", defaultValue: "Disable cove Browser")),
        ...
        contributions.append(
            CommandPaletteCommandContribution(
                ...
                title: constant(String(localized: "command.enableBrowser.title", defaultValue: "Enable cove Browser")),
                subtitle: constant(String(localized: "command.browserAvailability.subtitle", defaultValue: "Browser")),
        ...
        let coveConfigDefaultSubtitle = String(localized: "command.coveConfig.subtitle", defaultValue: "cove.json")
        for issue in coveConfigStore.configurationIssues {
            contributions.append(
                CommandPaletteCommandContribution(
                    commandId: commandPaletteCoveConfigIssueCommandID(issue),
                    title: constant(commandPaletteCoveConfigIssueTitle(issue)),
                    subtitle: constant(commandPaletteCoveConfigIssueSubtitle(issue)),
                    keywords: ["cove", "config", "json", "schema", "error", "warning"]
                )
            )
        }
        for action in coveConfigStore.paletteCustomActions() {
            let actionTitle = sanitizeCoveConfigPaletteText(action.title)
            let subtitleText = action.subtitle
                .map { sanitizeCoveConfigPaletteText($0) }
                .flatMap { $0.isEmpty ? nil : $0 }
                ?? coveConfigDefaultSubtitle
            contributions.append(
                CommandPaletteCommandContribution(
                    commandId: action.id,
    ...
    private func sanitizeCoveConfigPaletteText(_ text: String) -> String {
    ...
    private func commandPaletteCoveConfigIssueCommandID(_ issue: CoveConfigIssue) -> String {
        var hash: UInt64 = 1_469_598_103_934_665_603
        for byte in issue.id.utf8 {
            hash ^= UInt64(byte)
            hash &*= 1_099_511_628_211
        }
        return "palette.coveConfig.issue.\(String(hash, radix: 16))"
    }
    ...
    private func commandPaletteCoveConfigIssueTitle(_ issue: CoveConfigIssue) -> String {
        switch issue.kind {
        case .schemaError:
            return String(
                localized: "command.coveConfig.issue.schemaError.title",
                defaultValue: "cove.json Schema Error"
            )
        default:
            return String(
                localized: "command.coveConfig.issue.warning.title",
                defaultValue: "cove.json Configuration Warning"
            )
        }
    }

    private func commandPaletteCoveConfigIssueSubtitle(_ issue: CoveConfigIssue) -> String {
        let rawPath = issue.sourcePath.map {
            NSString(string: $0).abbreviatingWithTildeInPath
        } ?? issue.settingName
        let path = sanitizeCoveConfigPaletteText(rawPath)
        let detail = sanitizeCoveConfigPaletteText(commandPaletteCoveConfigIssueDetail(issue))
        guard !detail.isEmpty else { return path }
        let format = String(
            localized: "command.coveConfig.issue.subtitle",
            defaultValue: "%@: %@"
        )
        return String(format: format, path, detail)
    }

    private func commandPaletteCoveConfigIssueDetail(_ issue: CoveConfigIssue) -> String {
        switch issue.kind {
        case .schemaError:
            let format = String(
                localized: "command.coveConfig.issue.schemaError.detail",
                defaultValue: "%@"
            )
            let fallback = String(
                localized: "command.coveConfig.issue.schemaError.fallback",
                defaultValue: "Invalid cove.json"
            )
            return String(format: format, issue.message ?? fallback)
        case .newWorkspaceActionNotFound:
            let format = String(localized: "command.coveConfig.issue.newWorkspaceActionNotFound.detail", defaultValue: "%@ references missing action '%@'")
            return String(format: format, issue.settingName, issue.commandName ?? "")
        case .newWorkspaceCommandNotFound:
            let format = String(
                localized: "command.coveConfig.issue.newWorkspaceCommandNotFound.detail",
                defaultValue: "%@ references missing command '%@'"
            )
            return String(format: format, issue.settingName, issue.commandName ?? "")
        case .newWorkspaceCommandRequiresWorkspace:
            let format = String(
                localized: "command.coveConfig.issue.newWorkspaceCommandRequiresWorkspace.detail",
    ...
    private func registerCommandPaletteHandlers(_ registry: inout CommandPaletteHandlerRegistry) {
        ...
        registry.register(commandId: "palette.installCLI") {
            AppDelegate.shared?.installCoveCLIInPath(nil)
        }
        registry.register(commandId: "palette.uninstallCLI") {
            AppDelegate.shared?.uninstallCoveCLIInPath(nil)
        }
        registry.register(commandId: "palette.newTerminalTab") {
            if !executeConfiguredAction(id: CoveSurfaceTabBarBuiltInAction.newTerminal.configID) {
                tabManager.newSurface()
            }
        }
        registry.register(commandId: "palette.newBrowserTab") {
            if executeConfiguredAction(id: CoveSurfaceTabBarBuiltInAction.newBrowser.configID) {
                return
            }
...
#if DEBUG
            coveDebugLog("palette.openSettings.invoke")
#endif
            if let appDelegate = AppDelegate.shared {
                appDelegate.openPreferencesWindow(debugSource: "palette.openSettings")
            } else {
#if DEBUG
                coveDebugLog("palette.openSettings.missingAppDelegate fallback=1")
#endif
                AppDelegate.presentPreferencesWindow()
            }
        }
        registry.register(commandId: "palette.openCoveSettingsFile") {
#if DEBUG
            coveDebugLog("palette.openCoveSettingsFile.invoke")
#endif
            openCoveSettingsFileInEditor()
        }
        ...
        registry.register(commandId: "palette.terminalSplitRight") {
            if !executeConfiguredAction(id: CoveSurfaceTabBarBuiltInAction.splitRight.configID) {
                tabManager.createSplit(direction: .right)
            }
        }
        registry.register(commandId: "palette.terminalSplitDown") {
            if !executeConfiguredAction(id: CoveSurfaceTabBarBuiltInAction.splitDown.configID) {
...
#if DEBUG
                coveDebugLog("palette.equalizeSplits result=noSplitOrFailed workspaceId=\(workspace.id)")
#endif
            }
        }
        ...
        for issue in coveConfigStore.configurationIssues {
            let captured = issue
            registry.register(commandId: commandPaletteCoveConfigIssueCommandID(issue)) {
                openCoveConfigIssue(captured)
            }
        }
        for action in coveConfigStore.paletteCustomActions() {
            let captured = action
    ...
    private func openCoveConfigIssue(_ issue: CoveConfigIssue) {
    ...
    private func executeConfiguredAction(id: String) -> Bool {
        guard let action = coveConfigStore.resolvedAction(id: id) else {
            return false
        }
    ...
    @discardableResult
    private func executeConfiguredAction(_ action: CoveResolvedConfigAction) -> Bool {
        let baseCwd = configuredActionBaseCwd()
        return CoveConfigExecutor.execute(
            action: action,
            commands: coveConfigStore.loadedCommands,
            commandSourcePaths: coveConfigStore.commandSourcePaths,
            tabManager: tabManager,
            baseCwd: baseCwd,
            globalConfigPath: coveConfigStore.globalConfigPath
        )
    }
...
#if DEBUG
        coveDebugLog("palette.run commandId=\(command.id) dismissOnRun=\(command.dismissOnRun ? 1 : 0)")
#endif
    ...
    private func handleCommandPaletteBackdropClick(atContentPoint contentPoint: CGPoint) {
        ...
        if let clickedFocusTarget {
            ...
            coveDebugLog("palette.dismiss.backdrop focusTarget=nil")
        }
#endif
    ...
    private func commandPaletteBackdropFocusTarget(for responder: NSResponder) -> CommandPaletteRestoreFocusTarget? {
        if let terminalView = coveOwningGhosttyView(for: responder),
           let workspaceId = terminalView.tabId,
           let panelId = terminalView.terminalSurface?.id,
           tabManager.tabs.contains(where: { $0.id == workspaceId }) {
            return commandPaletteRestoreFocusTarget(
    ...
    private func applyUITestSidebarSelectionIfNeeded(tabs: [Workspace]) {
        ...
        guard let rawValue = env["COVE_UI_TEST_SIDEBAR_SELECTED_WORKSPACE_INDICES"]?
    ...
    private func openWorkspacePullRequestsInConfiguredBrowser() -> Bool {
        ...
        var openedCount = 0
        if BrowserLinkOpenSettings.openSidebarPullRequestLinksInCoveBrowser() {
...
private struct SidebarTabItemSettingsSnapshot: Equatable {
    ...
    let openPullRequestLinksInCoveBrowser: Bool
    let openPortLinksInCoveBrowser: Bool
    ...
    init(defaults: UserDefaults = .standard) {
        ...
        openPullRequestLinksInCoveBrowser = BrowserLinkOpenSettings.openSidebarPullRequestLinksInCoveBrowser(
            defaults: defaults
        )
        openPortLinksInCoveBrowser = BrowserLinkOpenSettings.openSidebarPortLinksInCoveBrowser(
            defaults: defaults
        )
...
private enum CoveGlassTokens {
    static let textPrimary = Color.white.opacity(0.92)
    static let textSecondary = Color.white.opacity(0.58)
    static let textTertiary = Color.white.opacity(0.36)
    static let textQuaternary = Color.white.opacity(0.20)
    static let accent = Color(nsColor: NSColor(srgbRed: 10.0 / 255.0, green: 132.0 / 255.0, blue: 1.0, alpha: 1.0))
    static let green = Color(nsColor: NSColor(srgbRed: 50.0 / 255.0, green: 215.0 / 255.0, blue: 75.0 / 255.0, alpha: 1.0))
    static let amber = Color(nsColor: NSColor(srgbRed: 1.0, green: 214.0 / 255.0, blue: 10.0 / 255.0, alpha: 1.0))
    static let purple = Color(nsColor: NSColor(srgbRed: 191.0 / 255.0, green: 90.0 / 255.0, blue: 242.0 / 255.0, alpha: 1.0))
...
struct VerticalTabsSidebar: View {
...
#if DEBUG
            coveDebugLog("sidebar.dragState.sidebar tab=\(debugShortSidebarTabId(newDraggedTabId))")
#endif
...
#if DEBUG
            coveDebugLog("sidebar.dragClear tab=\(debugShortSidebarTabId(draggedTabId)) reason=\(reason)")
#endif
            draggedTabId = nil
        }
        .onReceive(
    ...
    private func workspaceScrollContent(
        ...
        VStack(spacing: 0) {
            conductorWorkspaceOverview(tabs: renderContext.tabs)
    ...
    @MainActor
    private func conductorWorkspaceOverview(tabs: [Workspace]) -> some View {
        let registry = WorkspaceRegistry.liveProjectWorkspaceProjection(
            from: tabs,
            selectedWorkspaceID: tabManager.selectedTabId
        )
        let mountedIds = Set(tabs.map { $0.id })
        let model = ConductorWorkspaceSidebarModel(
            registry: registry,
            runtimeActiveWorkspaceIDs: mountedIds
        )

        return ConductorSidebarOverview(model: model)
            .padding(.horizontal, 10)
            .padding(.top, isMinimalMode ? 36 : 12)
            .padding(.bottom, 8)
    }
...
private struct ConductorSidebarOverview: View {
    let model: ConductorWorkspaceSidebarModel

    var body: some View {
        VStack(alignment: .leading, spacing: 8) {
            ConductorSidebarSearchRow()
            ConductorSidebarScopeRow(
                title: "All agents",
                count: model.activeAgents.count,
                symbolName: "globe",
                color: CoveGlassTokens.accent,
                isSelected: true
            )
            ConductorSidebarScopeRow(
                title: "Needs attention",
                count: model.needsAttention.count,
                symbolName: "bell",
                color: CoveGlassTokens.amber,
                isSelected: false
            )
            ConductorSidebarScopeRow(
                title: "Ready to review",
                count: model.repoGroups.count,
                symbolName: "checkmark.circle",
                color: CoveGlassTokens.green,
                isSelected: false
            )
            if !model.needsAttention.isEmpty {
                ConductorSidebarAttentionRow(count: model.needsAttention.count)
            }
        }
        .padding(.horizontal, 6)
        .padding(.top, 8)
        .padding(.bottom, 8)
        .accessibilityIdentifier("conductorSidebarOverview")
    }
}

private struct ConductorSidebarSearchRow: View {
    var body: some View {
        HStack(spacing: 7) {
            Image(systemName: "magnifyingglass")
                .font(.system(size: 11, weight: .medium))
                .foregroundStyle(CoveGlassTokens.textTertiary)
            ...
            Text("⌘F")
                ...
                .foregroundStyle(CoveGlassTokens.textQuaternary)
                .padding(.horizontal, 4)
                .padding(.vertical, 1)
                .background(Color.white.opacity(0.07), in: RoundedRectangle(cornerRadius: 3, style: .continuous))
                .coveGlassRounded(cornerRadius: 3)
        }
        .frame(height: 26)
        .padding(.horizontal, 10)
        .background(CoveGlassTokens.controlFill, in: RoundedRectangle(cornerRadius: 6, style: .continuous))
        .coveGlassRounded(cornerRadius: 6, interactive: true)
        .overlay {
...
private struct ConductorSidebarScopeRow: View {
    let title: String
    let count: Int
    ...
    var body: some View {
        HStack(spacing: 8) {
            Image(systemName: symbolName)
                .font(.system(size: 11, weight: .medium))
                .foregroundStyle(isSelected ? Color.white.opacity(0.82) : color.opacity(0.9))
                .frame(width: 13)
            Text(title)
                .font(.system(size: 12.5, weight: .medium))
                .foregroundStyle(isSelected ? CoveGlassTokens.textPrimary : CoveGlassTokens.textSecondary)
            Spacer(minLength: 0)
            if count > 0 {
                Text("\(count)")
                    .font(.system(size: 11.5, weight: .medium, design: .rounded))
                    .foregroundStyle(isSelected ? Color.white.opacity(0.70) : CoveGlassTokens.textTertiary)
            }
        }
        .lineLimit(1)
        .frame(height: 26)
        .padding(.horizontal, 10)
        .background(
            isSelected ? Color.white.opacity(0.070) : Color.white.opacity(0.012),
            in: RoundedRectangle(cornerRadius: 6, style: .continuous)
        )
        .coveGlassRounded(cornerRadius: 6, interactive: isSelected)
        .overlay {
            RoundedRectangle(cornerRadius: 6, style: .continuous)
                .strokeBorder(isSelected ? Color.white.opacity(0.12) : Color.clear, lineWidth: 0.5)
        }
    }
}

private struct ConductorSidebarAttentionRow: View {
    let count: Int
    ...
    var body: some View {
        ...
        .padding(.vertical, 4)
        .background(CoveGlassTokens.amber.opacity(0.10), in: Capsule(style: .continuous))
        .coveGlassCapsule(tint: CoveGlassTokens.amber.opacity(0.12))
    }
}
...
private enum FeedbackComposerSettings {
    ...
    static let endpointEnvironmentKey = "COVE_FEEDBACK_API_URL"
    static let defaultEndpoint = "https://cove.example.invalid/api/feedback"
...
private struct FeedbackComposerAppMetadata {
    ...
    static var current: FeedbackComposerAppMetadata {
        ...
        let env = ProcessInfo.processInfo.environment
        let commit = (infoDictionary["CoveCommit"] as? String).flatMap { value in
...
enum SidebarDragLifecycleNotification {
    static let stateDidChange = Notification.Name("cove.sidebarDragStateDidChange")
    static let requestClear = Notification.Name("cove.sidebarDragRequestClear")
...
#if DEBUG
        coveDebugLog("sidebar.dragFailsafe.schedule reason=\(reason)")
#endif
        let workItem = DispatchWorkItem { [weak self] in
#if DEBUG
            coveDebugLog("sidebar.dragFailsafe.fire reason=\(reason)")
#endif
...
#if DEBUG
        coveDebugLog("sidebar.dropOutside.entered tab=\(debugShortSidebarTabId(draggedTabId))")
#endif
    }

    func dropExited(info: DropInfo) {
#if DEBUG
        coveDebugLog("sidebar.dropOutside.exited tab=\(debugShortSidebarTabId(draggedTabId))")
#endif
    }
...
#if DEBUG
        coveDebugLog("sidebar.dropOutside.updated tab=\(debugShortSidebarTabId(draggedTabId)) op=move")
#endif
...
#if DEBUG
        coveDebugLog("sidebar.dropOutside.perform tab=\(debugShortSidebarTabId(draggedTabId))")
#endif
...
private struct SidebarFooterButtons: View {
    ...
    var body: some View {
        HStack(spacing: 4) {
            ...
            if updateViewModel.shouldShowSidebarUpdatePill {
...
private struct SidebarHelpMenuButton: View {
    private let docsURL = URL(string: "https://cove.example.invalid/docs")
    private let changelogURL = URL(string: "https://cove.example.invalid/docs/changelog")
    private let githubURL = URL(string: "https://github.com/manaflow-ai/cove-pending")
    private let githubIssuesURL = URL(string: "https://github.com/manaflow-ai/cove-pending/issues")
    ...
    private var helpPopover: some View {
        VStack(alignment: .leading, spacing: 2) {
            helpOptionButton(
                title: String(localized: "sidebar.help.welcome", defaultValue: "Welcome to cove!"),
                action: .welcome,
...
private struct TabItemView: View, Equatable {
    ...
    @State private var renameOriginalName: String = ""
    ...
    private var openSidebarPullRequestLinksInCoveBrowser: Bool {
        settings.openPullRequestLinksInCoveBrowser
    }

    private var openSidebarPortLinksInCoveBrowser: Bool {
        settings.openPortLinksInCoveBrowser
    }
    ...
    private func activeSecondaryColor(_ opacity: Double = 0.75) -> Color {
        usesInvertedActiveForeground
            ...
            : Color.white.opacity(0.58 * opacity / 0.75)
    }
    ...
    private var activeUnreadBadgeFillColor: Color {
        ...
        return usesInvertedActiveForeground ? Color.white.opacity(0.25) : coveAccentColor()
    }
    ...
    private var activeProgressFillColor: Color {
        usesInvertedActiveForeground ? Color.white.opacity(0.8) : coveAccentColor()
    }

    private var statusDotColor: Color {
        let snapshot = workspaceSnapshot
        if unreadCount > 0 {
            return CoveGlassTokens.accent
        }
        if snapshot.latestLog?.level == .error || tab.remoteConnectionState == .error {
            return CoveGlassTokens.amber
        }
        if snapshot.progress != nil || tab.remoteConnectionState == .connecting {
            return CoveGlassTokens.accent
        }
        if tab.isRemoteWorkspace || tab.hasActiveRemoteTerminalSessions {
            return CoveGlassTokens.purple
        }
        return isActive ? CoveGlassTokens.green : Color.white.opacity(0.18)
    }
    ...
    var body: some View {
        ...
        VStack(alignment: .leading, spacing: 4) {
            HStack(spacing: 8) {
                Circle()
                    ...
                    .shadow(color: statusDotColor.opacity(isActive ? 0.55 : 0), radius: 5)
                ...
                if editingWorkspaceId == tab.id {
                    ...
                    Text(workspaceSnapshot.title)
                        ...
                        .layoutPriority(1)
                        .onTapGesture(count: 2) {
                            renameText = workspaceSnapshot.title
                            renameOriginalName = workspaceSnapshot.title
                ...
                ZStack(alignment: .trailing) {
                    Button(action: {
                        #if DEBUG
                        coveDebugLog("sidebar.close workspace=\(tab.id.uuidString.prefix(5)) method=button")
                        #endif
        ...
        .overlay {
            MiddleClickCapture {
                #if DEBUG
                coveDebugLog("sidebar.close workspace=\(tab.id.uuidString.prefix(5)) method=middleClick")
                #endif
                tabManager.closeWorkspaceWithConfirmation(tab)
            }
        }
        ...
        .onDrag {
            #if DEBUG
            coveDebugLog("sidebar.onDrag tab=\(tab.id.uuidString.prefix(5))")
            #endif
            draggedTabId = tab.id
            dropIndicator = nil
    ...
    private func commitRename() {
        guard editingWorkspaceId == tab.id else { return }
        let trimmed = renameText.trimmingCharacters(in: .whitespacesAndNewlines)
        guard !trimmed.isEmpty else {
            editingWorkspaceId = nil
            return
        }
        guard trimmed.count <= 255, !trimmed.contains("/"), !trimmed.contains("\0") else {
            renameText = renameOriginalName
            editingWorkspaceId = nil
            return
        }
        tabManager.renamePermanentWorkspace(tabId: tab.id, name: trimmed)
        editingWorkspaceId = nil
    }
    ...
    private var workspaceContextMenu: some View {
        ...
        Button(String(localized: "contextMenu.deleteWorkspace", defaultValue: "Delete Workspace…")) {
            tabManager.deleteWorkspace(tab)
        }
    ...
    private func updateSelection() {
        ...
        coveDebugLog("sidebar.select workspace=\(tab.id.uuidString.prefix(5)) modifiers=\(modStr.isEmpty ? "none" : modStr.trimmingCharacters(in: .whitespaces))")
        #endif
    ...
    private func openPullRequestLink(_ url: URL) {
        updateSelection()
        if openSidebarPullRequestLinksInCoveBrowser {
    ...
    private func openPortLink(_ port: Int) {
        ...
        updateSelection()
        if openSidebarPortLinksInCoveBrowser {
            if tabManager.openBrowser(
                inWorkspace: tab.id,
                url: url,
                preferSplitRight: true,
...
private enum SidebarTabDragPayload {
    static let typeIdentifier = "com.cove.sidebar-tab-reorder"
    static let dropContentType = UTType(exportedAs: typeIdentifier)
    static let dropContentTypes: [UTType] = [dropContentType]
    private static let prefix = "cove.sidebar-tab."
...
private struct SidebarTabDropDelegate: DropDelegate {
    ...
    func validateDrop(info: DropInfo) -> Bool {
        ...
        #if DEBUG
        coveDebugLog("sidebar.validateDrop target=\(targetTabId?.uuidString.prefix(5) ?? "end") hasType=\(hasType) hasDrag=\(hasDrag)")
        #endif
    ...
    func dropEntered(info: DropInfo) {
        #if DEBUG
        coveDebugLog("sidebar.dropEntered target=\(targetTabId?.uuidString.prefix(5) ?? "end")")
        #endif
        dragAutoScrollController.updateFromDragLocation()
    ...
    func dropExited(info: DropInfo) {
#if DEBUG
        coveDebugLog("sidebar.dropExited target=\(targetTabId?.uuidString.prefix(5) ?? "end")")
#endif
    ...
    func performDrop(info: DropInfo) -> Bool {
        ...
        #if DEBUG
        coveDebugLog("sidebar.drop target=\(targetTabId?.uuidString.prefix(5) ?? "end")")
        #endif
        guard let draggedTabId else {
#if DEBUG
            coveDebugLog("sidebar.drop.abort reason=missingDraggedTab")
#endif
            return false
        }
        guard let fromIndex = tabManager.tabs.firstIndex(where: { $0.id == draggedTabId }) else {
#if DEBUG
            coveDebugLog("sidebar.drop.abort reason=draggedTabMissing tab=\(draggedTabId.uuidString.prefix(5))")
#endif
            return false
        }
...
#if DEBUG
            coveDebugLog("sidebar.drop.noop from=\(fromIndex) to=\(targetIndex)")
#endif
            syncSidebarSelection()
            return true
        }
...
#if DEBUG
        coveDebugLog("sidebar.drop.commit tab=\(draggedTabId.uuidString.prefix(5)) from=\(fromIndex) to=\(targetIndex)")
#endif
...
final class DraggableFolderNSView: NSView, NSDraggingSource {
    ...
    func draggingSession(_ session: NSDraggingSession, endedAt screenPoint: NSPoint, operation: NSDragOperation) {
        ...
        coveDebugLog("folder.dragEnd dir=\(directory) operation=\(operation.rawValue) screen=\(formatPoint(screenPoint)) nowMovable=\(nowMovable) windowOrigin=\(windowOrigin)")
        #endif
    }
    ...
    override func hitTest(_ point: NSPoint) -> NSView? {
        ...
        let nowMovable = window.map { String($0.isMovable) } ?? "nil"
        coveDebugLog("folder.hitTest point=\(formatPoint(point)) hit=\(hitDesc) imageViewHit=\(imageHit) returning=DraggableFolderNSView wasMovable=\(wasMovable) nowMovable=\(nowMovable)")
        #endif
        return self
    }
    ...
    override func mouseDown(with event: NSEvent) {
        ...
        let nowMovable = window.map { String($0.isMovable) } ?? "nil"
        let windowOrigin = window.map { formatPoint($0.frame.origin) } ?? "nil"
        coveDebugLog("folder.mouseDown dir=\(directory) point=\(formatPoint(localPoint)) firstResponder=\(responderDesc) wasMovable=\(wasMovable) nowMovable=\(nowMovable) windowOrigin=\(windowOrigin)")
        #endif
        ...
        coveDebugLog("folder.dragStart dir=\(directory) pasteboardItems=\(itemCount)")
        #endif
    }
    ...
    private func restoreWindowMovableStateIfNeeded() {
        ...
        coveDebugLog("folder.dragSuppression restore depth=\(depthAfter) nowMovable=\(nowMovable)")
        #endif
    }
...
private struct WindowBackdropLayer: View {
    ...
    private func backdrop(for policy: WindowBackdropPolicy) -> some View {
        ...
        case let .ghosttyTerminalBackdrop(color, opacity, _):
            ...
            switch role {
            case .windowRoot:
                CoveWindowRootBackdrop(baseColor: Color(nsColor: backdropColor))
...
private struct CoveWindowRootBackdrop: View {
    let baseColor: Color
...
enum SidebarPresetOption: String, CaseIterable, Identifiable {
    ...
    var tintHex: String {
        switch self {
        case .nativeSidebar: return "#09090F"
    ...
    var tintOpacity: Double {
        switch self {
        case .nativeSidebar: return 0.34
