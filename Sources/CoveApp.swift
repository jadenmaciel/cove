...
struct CoveApp: App {
    ...
    init() {
        Self.terminateIfAppHostedTestsAreNotExplicitlyAllowed()
        ...
        if let stored = defaults.string(forKey: SocketControlSettings.appStorageKey) {
            ...
            defaults.set(legacy ? SocketControlMode.coveOnly.rawValue : SocketControlMode.off.rawValue,
    ...
    private static func terminateIfAppHostedTestsAreNotExplicitlyAllowed() {
        let allowsAppHostedTests = getenv("COVE_ALLOW_APP_LAUNCH_TESTS")
            .map { String(cString: $0) == "1" } ?? false
        guard !allowsAppHostedTests else { return }
        guard getenv("XCTestConfigurationFilePath") != nil || getenv("XCTestSessionIdentifier") != nil else { return }

        let message = "error: refusing to launch Cove.app under XCTest; set COVE_ALLOW_APP_LAUNCH_TESTS=1 to run app-hosted tests intentionally"
        fputs("\(message)\n", stderr)
        fflush(stderr)
        NSLog("%@", message)
        Darwin.exit(86)
    ...
    private static func terminateForMissingLaunchTag() -> Never {
        let message = "error: refusing to launch untagged Cove debug build; start with ./scripts/reload.sh --tag <name> (or set COVE_TAG for test harnesses)"
        fputs("\(message)\n", stderr)
        fflush(stderr)
        NSLog("%@", message)
        Darwin.exit(64)
    ...
    var body: some Scene {
        WindowGroup {
            MainWindowBootstrapView()
                .coveAppearanceColorScheme(appearanceMode)
                .onAppear {
                    ...
                    if ProcessInfo.processInfo.environment["COVE_UI_TEST_MODE"] == "1" {
                        UpdateLogStore.shared.append("ui test: CoveApp onAppear")
                    }
#endif
        ...
        .commands {
            CommandGroup(replacing: .appSettings) {
                ...
                Button(String(localized: "menu.app.openCoveSettingsFile", defaultValue: "Open cove.json")) {
            ...
            CommandGroup(replacing: .appInfo) {
                Button(String(localized: "menu.app.about", defaultValue: "About cove")) {
                    showAboutPanel()
                }
            ...
            CommandGroup(replacing: .appTermination) {
                splitCommandButton(title: String(localized: "menu.quitCove", defaultValue: "Quit cove"), shortcut: menuShortcut(for: .quit)) {
                    NSApp.terminate(nil)
                }
            }
...
#if DEBUG
                        coveDebugLog("find.menu Cmd+F fired")
#endif
    ...
    private var windowAndViewCommands: some Commands {
        ...
        CommandGroup(after: .toolbar) {
            ...
            splitCommandButton(title: String(localized: "menu.view.nextWorkspaceCycle", defaultValue: "Next Workspace (Ctrl+Tab)"), shortcut: menuShortcut(for: .nextWorkspace)) {
                activeTabManager.selectNextTab()
            }

            splitCommandButton(title: String(localized: "menu.view.previousWorkspaceCycle", defaultValue: "Previous Workspace (Ctrl+Shift+Tab)"), shortcut: menuShortcut(for: .prevWorkspace)) {
                activeTabManager.selectPreviousTab()
            }
    ...
    private func applyAppearance() {
        let mode = AppearanceSettings.applyStoredMode(
            ...
            source: "CoveApp.appearanceModeChanged"
        )
    ...
    private static func applyAppearance(_ mode: AppearanceMode, duringLaunch: Bool = false) {
        AppearanceSettings.applyLiveMode(
            mode,
            source: duringLaunch ? "CoveApp.launch" : "CoveApp.applyAppearance",
    ...
    private func closePanelOrWindow() {
        if let window = NSApp.keyWindow ?? NSApp.mainWindow,
           coveWindowShouldOwnCloseShortcut(window) {
...
private struct MainWindowBootstrapView: View {
    var body: some View {
        Color.clear
            ...
            .background(WindowAccessor { window in
                window.identifier = NSUserInterfaceItemIdentifier("cove.bootstrap")
...
private let coveAuxiliaryWindowIdentifiers: Set<String> = [
    "cove.settings",
    "cove.about",
    "cove.licenses",
    "cove.browser-popup",
    "cove.aboutTitlebarDebug",
    "cove.debugWindowControls",
    "cove.browserImportHintDebug",
    "cove.sidebarDebug",
    "cove.menubarDebug",
    "cove.backgroundDebug",
    "cove.startupAppearanceDebug",
    "cove.bonsplitTabBarDebug",
]

/// Returns whether the given window should handle the standard close shortcut
/// as a standalone auxiliary window instead of routing it through workspace or
/// panel-close behavior.
func coveWindowShouldOwnCloseShortcut(_ window: NSWindow?) -> Bool {
    guard let identifier = window?.identifier?.rawValue else { return false }
    return coveAuxiliaryWindowIdentifiers.contains(identifier)
}
...
private final class AboutTitlebarDebugStore: ObservableObject {
    ...
    private func ensureToolbar(on window: NSWindow, kind: AboutWindowKind) {
        ...
        let identifier = NSToolbar.Identifier("cove.debug.titlebar.\(kind.rawValue)")
...
private final class AboutTitlebarDebugWindowController: NSWindowController, NSWindowDelegate {
    ...
    private init() {
        ...
        window.identifier = NSUserInterfaceItemIdentifier("cove.aboutTitlebarDebug")
        window.center()
    ...
    private func editor(for kind: AboutWindowKind) -> some View {
        ...
        return GroupBox(kind.displayTitle) {
            VStack(alignment: .leading, spacing: 10) {
                ...
                Text("When disabled, cove uses normal default titlebar behavior for this window.")
                    .font(.caption)
...
private final class DebugWindowControlsWindowController: NSWindowController, NSWindowDelegate {
    ...
    private init() {
        ...
        window.isMovableByWindowBackground = true
        window.isReleasedWhenClosed = false
        window.identifier = NSUserInterfaceItemIdentifier("cove.debugWindowControls")
        window.center()
...
private final class BrowserImportHintDebugWindowController: NSWindowController, NSWindowDelegate {
    ...
    private init() {
        ...
        window.isMovableByWindowBackground = true
        window.isReleasedWhenClosed = false
        window.identifier = NSUserInterfaceItemIdentifier("cove.browserImportHintDebug")
        window.center()
...
private final class BrowserProfilePopoverDebugWindowController: NSWindowController, NSWindowDelegate {
    ...
    private init() {
        ...
        window.isMovableByWindowBackground = true
        window.isReleasedWhenClosed = false
        window.identifier = NSUserInterfaceItemIdentifier("cove.browserProfilePopoverDebug")
        window.center()
...
private final class AboutWindowController: NSWindowController, NSWindowDelegate {
    ...
    private init() {
        ...
        window.isReleasedWhenClosed = false
        window.identifier = NSUserInterfaceItemIdentifier("cove.about")
        window.center()
...
private final class AcknowledgmentsWindowController: NSWindowController, NSWindowDelegate {
    ...
    private init() {
        ...
        window.identifier = NSUserInterfaceItemIdentifier("cove.licenses")
        window.center()
...
enum SettingsWindowPresenter {
    ...
    static let windowIdentifier = "cove.settings"
...
#if DEBUG
        coveDebugLog("settings.window.show path=swiftuiWindow")
#endif
...
private final class FileExplorerStyleDebugWindowController: NSWindowController, NSWindowDelegate {
    ...
    private init() {
        ...
        window.isMovableByWindowBackground = true
        window.isReleasedWhenClosed = false
        window.identifier = NSUserInterfaceItemIdentifier("cove.fileExplorerStyleDebug")
        window.center()
...
private final class SidebarDebugWindowController: NSWindowController, NSWindowDelegate {
    ...
    private init() {
        ...
        window.isMovableByWindowBackground = true
        window.isReleasedWhenClosed = false
        window.identifier = NSUserInterfaceItemIdentifier("cove.sidebarDebug")
        window.center()
...
private struct AboutPanelView: View {
    ...
    private var commit: String? {
        if let value = Bundle.main.infoDictionary?["CoveCommit"] as? String, !value.isEmpty {
            return value
        }
        let env = ProcessInfo.processInfo.environment["COVE_COMMIT"] ?? ""
        ...
        VStack(alignment: .center) {
            ...
            VStack(alignment: .center, spacing: 32) {
                VStack(alignment: .center, spacing: 8) {
                    Text(String(localized: "about.appName", defaultValue: "cove"))
                        .bold()
                        .font(.title)
                ...
                VStack(spacing: 2) {
                    ...
                    let commitURL = commit.flatMap { hash in
                        URL(string: "https://github.com/manaflow-ai/cove-pending/commit/\(hash)")
                    }
...
private final class MenuBarExtraDebugWindowController: NSWindowController, NSWindowDelegate {
    ...
    private init() {
        ...
        window.isMovableByWindowBackground = true
        window.isReleasedWhenClosed = false
        window.identifier = NSUserInterfaceItemIdentifier("cove.menubarDebug")
        window.center()
...
private final class SplitButtonLayoutDebugWindowController: NSWindowController, NSWindowDelegate {
    ...
    private init() {
        ...
        window.isMovableByWindowBackground = true
        window.isReleasedWhenClosed = false
        window.identifier = NSUserInterfaceItemIdentifier("cove.splitButtonLayoutDebug")
        window.center()
...
private final class TabBarBackdropLabWindowController: NSWindowController, NSWindowDelegate {
    ...
    private init() {
        ...
        window.identifier = NSUserInterfaceItemIdentifier("cove.tabBarBackdropLab")
        window.center()
...
private struct TabBarBackdropLabSample: View {
    ...
    private static func makeController(for variant: TabBarBackdropLabVariant) -> BonsplitController {
        ...
        let titles = [
            ...
            String(localized: "debug.tabBarBackdropLab.tab.terminalTransparency", defaultValue: "cove terminal transparency"),
...
private struct TabBarBackdropLabTerminalPane: View {
    ...
    var body: some View {
        ZStack(alignment: .topLeading) {
            ...
            VStack(alignment: .leading, spacing: 4) {
                Text("\(String(localized: "debug.tabBarBackdropLab.terminal.prompt", defaultValue: "lawrence in ~/cove")) \(title)")
...
private final class BackgroundDebugWindowController: NSWindowController, NSWindowDelegate {
    ...
    private init() {
        ...
        window.isMovableByWindowBackground = true
        window.isReleasedWhenClosed = false
        window.identifier = NSUserInterfaceItemIdentifier("cove.backgroundDebug")
        window.center()
    ...
    private func updateWindowGlassTint() {
        let window: NSWindow? = {
            if let key = NSApp.keyWindow,
               ...
               raw == "cove.main" || raw.hasPrefix("cove.main.") {
                return key
            }
            return NSApp.windows.first(where: {
                guard let raw = $0.identifier?.rawValue else { return false }
                return raw == "cove.main" || raw.hasPrefix("cove.main.")
            })
        }()
        guard let window else { return }
...
private final class StartupAppearanceDebugWindowController: NSWindowController, NSWindowDelegate {
    ...
    private init() {
        ...
        window.isMovableByWindowBackground = true
        window.isReleasedWhenClosed = false
        window.identifier = NSUserInterfaceItemIdentifier("cove.startupAppearanceDebug")
        window.center()
...
enum AppIconSettings {
    ...
    private static let dockTileIconDidChangeNotification = Notification.Name("com.coveIconDidChange")
    ...
    private static func isRunningUnderXCTest(_ env: [String: String] = ProcessInfo.processInfo.environment) -> Bool {
        ...
        if env.keys.contains(where: { $0.hasPrefix("COVE_UI_TEST_") }) { return true }
        return false
    }
...
enum WelcomeSettings {
    static let shownKey = "coveWelcomeShown"
}
...
enum CmdClickMarkdownRouteSettings {
    static let key = "openMarkdownInCoveViewer"
...
enum PreferredEditorSettings {
    ...
    static func open(_ url: URL) {
        if CoveUITestCapture.appendLineIfConfigured(
            envKey: "COVE_UI_TEST_CAPTURE_OPEN_PATH",
            line: url.path
        ) {
            return
        }
...
enum CoveRuntimeDebugCapture {
    ...
    private static let configuration: Configuration? = {
        let env = ProcessInfo.processInfo.environment
        guard let baseURLString = env["COVE_RUNTIME_DEBUG_BASE_URL"]?.trimmingCharacters(in: .whitespacesAndNewlines),
              let baseURL = URL(string: baseURLString),
              let token = env["COVE_RUNTIME_DEBUG_TOKEN"]?.trimmingCharacters(in: .whitespacesAndNewlines),
              !token.isEmpty,
              let sessionID = env["COVE_RUNTIME_DEBUG_SESSION_ID"]?.trimmingCharacters(in: .whitespacesAndNewlines),
...
func openCoveSettingsFileInEditor() {
...
struct SettingsView: View {
    ...
    private let shortcutChordsDocsURL = URL(string: "https://cove.example.invalid/docs/keyboard-shortcuts#shortcut-chords")!
    private let settingsJSONDocsURL = URL(string: "https://cove.example.invalid/docs/configuration#cove-json")!
    @Environment(\.openWindow) private var openWindow
    ...
    @AppStorage(CmdClickMarkdownRouteSettings.key) private var openMarkdownInCoveViewer = CmdClickMarkdownRouteSettings.defaultValue
    @AppStorage("covePortBase") private var covePortBase = 9100
    @AppStorage("covePortRange") private var covePortRange = 10
    ...
    @AppStorage(BrowserLinkOpenSettings.openTerminalLinksInCoveBrowserKey) private var openTerminalLinksInCoveBrowser = BrowserLinkOpenSettings.defaultOpenTerminalLinksInCoveBrowser
    @AppStorage(BrowserLinkOpenSettings.interceptTerminalOpenCommandInCoveBrowserKey)
    private var interceptTerminalOpenCommandInCoveBrowser = BrowserLinkOpenSettings.initialInterceptTerminalOpenCommandInCoveBrowserValue()
    ...
    @AppStorage(BrowserLinkOpenSettings.openSidebarPullRequestLinksInCoveBrowserKey)
    private var openSidebarPullRequestLinksInCoveBrowser = BrowserLinkOpenSettings.defaultOpenSidebarPullRequestLinksInCoveBrowser
    @AppStorage(BrowserLinkOpenSettings.openSidebarPortLinksInCoveBrowserKey)
    private var openSidebarPortLinksInCoveBrowser = BrowserLinkOpenSettings.defaultOpenSidebarPortLinksInCoveBrowser
    ...
    private var paneFirstClickFocusSubtitle: String {
        if paneFirstClickFocusEnabled {
            return String(
                ...
                defaultValue: "When cove is inactive, clicking a pane activates the window and focuses that pane in one click."
            )
        }
        return String(
            localized: "settings.app.paneFirstClickFocus.subtitleOff",
            defaultValue: "When cove is inactive, the first click only activates the window. Click again to focus the pane."
        )
    }
    ...
    private var browserEnabledSettingsRows: some View {
        SettingsCardRow(
            ...
            String(localized: "settings.browser.enabled", defaultValue: "Enable cove Browser"),
...
#if DEBUG
        coveDebugLog("notification.ui enableTapped state=\(state)")
#endif
            ...
            ScrollView {
                VStack(alignment: .leading, spacing: 14) {
                    ...
                    SettingsCard {
                        SettingsCardRow(
                            ...
                            subtitle: appLanguage != LanguageSettings.languageAtLaunch.rawValue
                                ? String(localized: "settings.app.language.restartSubtitle", defaultValue: "Restart cove to apply")
                                : nil,
                        ...
                        SettingsCardRow(
                            ...
                            subtitle: String(
                                ...
                                defaultValue: "Open the cove config, standalone Ghostty config, and merged preview in one utility window."
                            ),
                        ...
                        SettingsCardRow(
                            configurationReview: .json("app.openMarkdownInCoveViewer"),
                            String(localized: "settings.app.openMarkdownInCoveViewer", defaultValue: "Open Markdown in cove Viewer"),
                            subtitle: String(localized: "settings.app.openMarkdownInCoveViewer.subtitle", defaultValue: "Cmd-clicking .md/.markdown/.mkd/.mdx files opens the cove markdown viewer panel instead of the preferred editor.")
                        ) {
                            Toggle("", isOn: $openMarkdownInCoveViewer)
                                .labelsHidden()
                                .controlSize(.small)
                                .accessibilityLabel(
                                    String(localized: "settings.app.openMarkdownInCoveViewer", defaultValue: "Open Markdown in cove Viewer")
                        ...
                        SettingsCardRow(
                            ...
                            subtitle: String(localized: "settings.app.menuBarOnly.subtitle", defaultValue: "Hide the Dock icon and Cmd+Tab entry. Use the menu bar item to show cove.")
                        ) {
                        ...
                        SettingsCardRow(
                            ...
                            subtitle: String(localized: "settings.app.showInMenuBar.subtitle", defaultValue: "Keep cove in the menu bar for unread notifications and quick actions.")
                        ) {
                        ...
                        SettingsCardRow(
                            ...
                            subtitle: String(localized: "settings.notifications.paneFlash.subtitle", defaultValue: "Briefly flash a blue outline when cove highlights a pane.")
                        ) {
                        ...
                        SettingsCardRow(
                            ...
                            subtitle: String(localized: "settings.notifications.command.subtitle", defaultValue: "Run a shell command when a notification arrives. $COVE_NOTIFICATION_TITLE, $COVE_NOTIFICATION_SUBTITLE, $COVE_NOTIFICATION_BODY are set.")
                        ) {
                        ...
                        SettingsCardRow(
                            ...
                            subtitle: sendAnonymousTelemetry != telemetryValueAtLaunch
                                ...
                                : String(localized: "settings.app.telemetry.subtitle", defaultValue: "Share anonymized crash and usage data to help improve cove.")
                        ) {
                    ...
                    SettingsCard {
                        SettingsCardRow(
                            ...
                            subtitle: showTerminalScrollBar
                                ? String(localized: "settings.terminal.scrollBar.subtitleOn", defaultValue: "Shows the right-edge terminal scroll bar in shell scrollback. cove hides it automatically for alternate-screen style TUI surfaces and you can also disable it per workspace.")
                    ...
                    SettingsCard {
                        ...
                        SettingsCardDivider()
                        SettingsCardRow(
                            configurationReview: .json("sidebar.openPullRequestLinksInCoveBrowser"),
                            String(localized: "settings.app.openSidebarPRLinks", defaultValue: "Open Sidebar PR Links in cove Browser"),
                            subtitle: !sidebarShowPullRequest ? String(localized: "settings.app.openSidebarPRLinks.subtitleHidden", defaultValue: "Enable sidebar PR visibility to choose where PR links open.") : (!sidebarMakePullRequestClickable ? String(localized: "settings.app.openSidebarPRLinks.subtitleDisabled", defaultValue: "Enable sidebar PR clickability to choose where PR links open.") : (openSidebarPullRequestLinksInCoveBrowser ? String(localized: "settings.app.openSidebarPRLinks.subtitleOn", defaultValue: "Clicks open inside cove browser.") : String(localized: "settings.app.openSidebarPRLinks.subtitleOff", defaultValue: "Clicks open in your default browser.")))
                        ) { Toggle("", isOn: $openSidebarPullRequestLinksInCoveBrowser).labelsHidden().controlSize(.small) }
                        .disabled(sidebarHideAllDetails || !sidebarShowPullRequest || !sidebarMakePullRequestClickable)
                        SettingsCardDivider()
                        SettingsCardRow(
                            configurationReview: .json("sidebar.openPortLinksInCoveBrowser"),
                            String(localized: "settings.app.openSidebarPortLinks", defaultValue: "Open Sidebar Port Links in cove Browser"),
                            subtitle: openSidebarPortLinksInCoveBrowser
                                ? String(localized: "settings.app.openSidebarPortLinks.subtitleOn", defaultValue: "Port clicks open inside cove browser.")
                                : String(localized: "settings.app.openSidebarPortLinks.subtitleOff", defaultValue: "Port clicks open in your default browser.")
                        ) {
                            Toggle("", isOn: $openSidebarPortLinksInCoveBrowser)
                                .labelsHidden()
                                .controlSize(.small)
                        }
                        .disabled(sidebarHideAllDetails)
                    ...
                    SettingsCard {
                        ...
                        SettingsCardNote(String(localized: "settings.automation.socketOverrides.note", defaultValue: "Overrides: COVE_SOCKET_ENABLE, COVE_SOCKET_MODE, and COVE_SOCKET_PATH (set COVE_ALLOW_SOCKET_OVERRIDE=1 for stable/nightly builds)."))
                    }
                    ...
                    SettingsCard {
                        SettingsCardRow(
                            ...
                            subtitle: claudeCodeHooksEnabled
                                ...
                                : String(localized: "settings.automation.claudeCode.subtitleOff", defaultValue: "Claude Code runs without cove integration.")
                        ) {
                        ...
                        SettingsCardNote(String(localized: "settings.automation.claudeCode.note", defaultValue: "When enabled, cove wraps the claude command to inject session tracking and notification hooks. Disable if you prefer to manage Claude Code hooks yourself."))
                    }
                    ...
                    SettingsCard {
                        SettingsCardRow(
                            ...
                            subtitle: cursorHooksEnabled
                                ...
                                : String(localized: "settings.automation.cursor.subtitleOff", defaultValue: "Cursor runs without cove integration.")
                        ) {
                        ...
                        SettingsCardNote(String(localized: "settings.automation.cursor.note", defaultValue: "Hooks must be installed with `cove hooks cursor install`. They no-op outside cove terminals."))
                    }
                    ...
                    SettingsCard {
                        SettingsCardRow(
                            ...
                            subtitle: geminiHooksEnabled
                                ...
                                : String(localized: "settings.automation.gemini.subtitleOff", defaultValue: "Gemini runs without cove integration.")
                        ) {
                        ...
                        SettingsCardNote(String(localized: "settings.automation.gemini.note", defaultValue: "Hooks must be installed with `cove hooks gemini install`. They no-op outside cove terminals."))
                    }

                    SettingsCard {
                        SettingsCardRow(configurationReview: .json("automation.portBase"), String(localized: "settings.automation.portBase", defaultValue: "Port Base"), subtitle: String(localized: "settings.automation.portBase.subtitle", defaultValue: "Starting port for COVE_PORT env var."), controlWidth: pickerColumnWidth) {
                            TextField("", value: $covePortBase, format: .number)
                                .textFieldStyle(.roundedBorder)
                                .multilineTextAlignment(.trailing)
                        }
                        ...
                        SettingsCardRow(configurationReview: .json("automation.portRange"), String(localized: "settings.automation.portRange", defaultValue: "Port Range Size"), subtitle: String(localized: "settings.automation.portRange.subtitle", defaultValue: "Number of ports per workspace."), controlWidth: pickerColumnWidth) {
                            TextField("", value: $covePortRange, format: .number)
                                .textFieldStyle(.roundedBorder)
                                .multilineTextAlignment(.trailing)
                        }
                        ...
                        SettingsCardNote(String(localized: "settings.automation.port.note", defaultValue: "Each workspace gets COVE_PORT and COVE_PORT_END env vars with a dedicated port range. New terminals inherit these values."))
                    }
                    ...
                    SettingsCard {
                        ...
                        SettingsCardRow(
                            configurationReview: .json("browser.openTerminalLinksInCoveBrowser"),
                            String(localized: "settings.browser.openTerminalLinks", defaultValue: "Open Terminal Links in cove Browser"),
                            ...
                            Toggle("", isOn: $openTerminalLinksInCoveBrowser)
                                .labelsHidden()
                                .controlSize(.small)
                        }
                        ...
                        SettingsCardRow(
                            configurationReview: .json("browser.interceptTerminalOpenCommandInCoveBrowser"),
                            ...
                            Toggle("", isOn: $interceptTerminalOpenCommandInCoveBrowser)
                                .labelsHidden()
                                .controlSize(.small)
                        }

                        if openTerminalLinksInCoveBrowser || interceptTerminalOpenCommandInCoveBrowser {
                            SettingsCardDivider()
                            ...
                            VStack(alignment: .leading, spacing: 6) {
                                SettingsCardRow(
                                    ...
                                    subtitle: String(localized: "settings.browser.hostWhitelist.subtitle", defaultValue: "Applies to terminal link clicks and intercepted `open https://...` calls. Only these hosts open in cove. Others open in your default browser. One host or wildcard per line (for example: example.com, *.internal.example). Leave empty to open all hosts in cove.")
                                ) {
                                    EmptyView()
                                }
                        ...
                        VStack(alignment: .leading, spacing: 8) {
                            ...
                            Text(String(localized: "settings.browser.httpAllowlist.description", defaultValue: "Controls which HTTP (non-HTTPS) hosts can open in cove without a warning prompt. Defaults include localhost, 127.0.0.1, ::1, 0.0.0.0, and *.localtest.me."))
                                .font(.caption)
                                .foregroundStyle(.secondary)
                    ...
                    SettingsCard {
                        SettingsCardRow(
                            ...
                            subtitle: String(localized: "settings.shortcuts.chords.subtitle", defaultValue: "Add tmux-style multi-step shortcuts in cove.json, for example [\"ctrl+b\", \"c\"]."),
                            ...
                            HStack(spacing: 8) {
                                ...
                                Button(String(localized: "settings.app.settingsFile.openButton", defaultValue: "Open cove.json")) {
                                    openCoveSettingsFileInEditor()
                                }
                                .buttonStyle(.bordered)
                                .controlSize(.small)
                    ...
                    SettingsCard {
                        ...
                        SettingsCardNote(
                            String(
                                ...
                                defaultValue: "Edit cove.json to add or remove named colors. \"Choose Custom Color...\" still adds local Custom N entries."
                            )
                        )
                        ...
                        if workspaceTabPaletteEntries.isEmpty {
                            SettingsCardNote(
                                String(
                                    ...
                                    defaultValue: "No palette entries. Add colors in cove.json or use \"Choose Custom Color...\" from a workspace context menu."
                                )
                            )
                        } else {
                    ...
                    SettingsSectionHeader(title: String(localized: "settings.section.settingsJSON", defaultValue: "cove.json"))
                    ...
                    SettingsCard {
                        SettingsCardRow(
                            ...
                            subtitle: String(localized: "settings.settingsJSON.file.subtitle", defaultValue: "Edit cove-owned app settings, shortcuts, automation, sidebar, notifications, and browser behavior."),
                            controlWidth: 330,
    ...
    private func resetAllSettings() {
        ...
        openMarkdownInCoveViewer = CmdClickMarkdownRouteSettings.defaultValue
        ...
        openTerminalLinksInCoveBrowser = BrowserLinkOpenSettings.defaultOpenTerminalLinksInCoveBrowser
        interceptTerminalOpenCommandInCoveBrowser = BrowserLinkOpenSettings.defaultInterceptTerminalOpenCommandInCoveBrowser
        ...
        openSidebarPullRequestLinksInCoveBrowser = BrowserLinkOpenSettings.defaultOpenSidebarPullRequestLinksInCoveBrowser
        openSidebarPortLinksInCoveBrowser = BrowserLinkOpenSettings.defaultOpenSidebarPortLinksInCoveBrowser
...
private struct AuthSettingsRow: View {
    ...
    private var subtitleText: String? {
        ...
        return String(
            ...
            defaultValue: "Sign in with your cove account to enable sync across devices."
        )
    }
...
private enum SettingsConfigurationReview: Equatable {
    ...
    func validate(file: StaticString = #fileID, line: UInt = #line) {
        guard case .settingsFile(let paths) = self else { return }
        let unknownPaths = paths.filter { !CoveSettingsFileStore.supportedSettingsJSONPaths.contains($0) }
        precondition(
            unknownPaths.isEmpty,
            "Unknown cove.json settings path(s): \(unknownPaths.joined(separator: ", "))",
            file: file,
            line: line
        )
    }
    ...
    private var enableSubtitle: String {
        if isEnabled {
            return String(
                ...
                defaultValue: "Press the shortcut from any app to show or hide all cove windows."
            )
        }
        return String(
            localized: "settings.globalHotkey.enable.subtitleOff",
            defaultValue: "Turn this on to show or hide all cove windows from any app."
        )
    }
